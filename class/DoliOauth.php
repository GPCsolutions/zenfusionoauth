 ...
 * Login URI
 */
define('GOOGLE_SERVICE_LOGIN_URL', 'https://www.google.com/accounts/ServiceLogin');
/**
 ...
 */
define('GOOGLE_CONTACTS_REVOQUE_TOKEN', 'https://www.google.com/accounts/AuthSubRevokeToken');
/**
 * Gmail revoke token URI
 */
...
class DoliOauth extends OAuth {
  ...
  public function revokeToken() {
    ...
    $this->fetch(GOOGLE_CONTACTS_REVOQUE_TOKEN, "", OAUTH_HTTP_METHOD_GET, array("Authorization" => ""));
  }
  ...
  public function deleteContact($idrecordgoogle) {
    $this->setAuthType(OAUTH_AUTH_TYPE_AUTHORIZATION);
    $this->fetch($idrecordgoogle, "", OAUTH_HTTP_METHOD_DELETE, array("If-Match" => "*", "GData-Version" => GDATA_VERSION));
  ...
  public function deleteContactOverride($idrecordgoogle) {
    $this->setAuthType(OAUTH_AUTH_TYPE_AUTHORIZATION);
    $this->fetch($idrecordgoogle, "", OAUTH_HTTP_METHOD_POST, array("X-HTTP-Methode-Override" => "DELETE", "If-Match" => "*", "GData-Version" => GDATA_VERSION));
    $response = $this->getLastResponseInfo();
    if ($response['http_code'] == 200) {
      return 1;
    }
  ...
  public function updateContact(EntreeTiers $object, $idrecordgoogle) {
    ...
    $this->setAuthType(OAUTH_AUTH_TYPE_AUTHORIZATION);
    $this->fetch($editurl, $object->saveXML(), OAUTH_HTTP_METHOD_PUT, array("Content-Type" => "application/atom+xml", "If-Match" => "*", "GData-Version" => GDATA_VERSION));
    $response = $this->getLastResponseInfo();
    if ($response['http_code'] == 200) {
      return 1;
    } // If firewalls troubleshooting try
  ...
  public function updateContactOverride(EntreeTiers $object, $idrecordgoogle) {
    ...
    $editurl = str_replace('base', 'full', $idrecordgoogle);
    $this->setAuthType(OAUTH_AUTH_TYPE_AUTHORIZATION);
    $this->fetch($editurl, $object->saveXML(), OAUTH_HTTP_METHOD_PUT, array("Content-Type" => "application/atom+xml", "X-HTTP-Methode-Override" => "DELETE", "If-Match" => "*", "GData-Version" => GDATA_VERSION));
    $response = $this->getLastResponseInfo();
    if ($response['http_code'] == 200) {
      return 1;
    }
